...
def main():
    ...
    if not sc_id or sc_id in ['None', 'nan', '']:
        ...
        company_options['display'] = company_options['company_name'] + " (" + company_options['scId'].astype(str) + ")"
    ...
    with price_col1:
        if cost and str(cost) not in ["-", "None", "nan"]:
            ...
            st.markdown(f"<h1 style='color:{price_color}; margin:0;'>₹{cost}</h1>",
    ...
    with price_col2:
        if price_change_value:
            ...
            st.markdown(f"<h2 style='color:{change_color};'>{change_icon} {price_change_value}</h2>",
                       unsafe_allow_html=True)
    ...
    metrics = [
        ...
        ("Cost", f"₹{data.get('cost', '-')}" if data.get("cost") else "-", "Current Price"),
    ...
    with col_right:
        ...
        if analyst_ratings and isinstance(analyst_ratings, list):
            ...
            try:
                ...
                if not ratings_df.empty and "name" in ratings_df.columns and "value" in ratings_df.columns:
