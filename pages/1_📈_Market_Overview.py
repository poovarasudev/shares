    ...
    for display_name, col_key, _, _ in COLUMN_DEFINITIONS:
        ...
        if col_key == "company_url":
            config[col_key] = st.column_config.LinkColumn(
                display_name,
                help="Click to open in MoneyControl",
                display_text="View"
            )
        elif col_key == "cost":
...
def render_filters(df: pd.DataFrame) -> tuple[pd.DataFrame, int, int]:
    ...
    if search_query:
        mask_name = filtered_df['company_name'].astype(str).str.lower().str.contains(search_query, na=False)
        mask_scid = filtered_df['scId'].astype(str).str.lower().str.contains(search_query, na=False)
    ...
    def apply_multiselect(column_name: str, label: str, default_values: list = None):
        ...
        if column_name in df.columns:
            ...
            default = default_values if default_values and all(v in options for v in default_values) else []
            selected = st.sidebar.multiselect(label, options, default=default, key=f"filter_{column_name}")
            if selected:
    ...
    if 'analyst_count' in df.columns and st.sidebar.checkbox("Filter by Analyst Count", key="filter_analyst_count_cb"):
    ...
    # M-Score filter
    if 'm_score' in df.columns and st.sidebar.checkbox("Filter by M-Score", key="filter_mscore_cb"):
        ...
        filtered_df = filtered_df[
            (filtered_df['m_score'] >= m_min) & 
            (filtered_df['m_score'] <= m_max)
        ]
    
    # PE filter
    if 'ttm_pe' in df.columns and st.sidebar.checkbox("Filter by PE Ratio", key="filter_pe_cb"):
        ...
        filtered_df = filtered_df[
            (filtered_df['ttm_pe'] >= pe_min) & 
            (filtered_df['ttm_pe'] <= pe_max)
        ]
...
def render_pagination(total_items: int, items_per_page: int) -> Tuple[int, int]:
    ...
    with col1:
        if st.button("⏮️", disabled=st.session_state.current_page == 1, 
                    width="stretch", help="First page"):
            st.session_state.current_page = 1
            st.rerun()
    
    with col2:
        if st.button("◀️", disabled=st.session_state.current_page == 1,
                    width="stretch", help="Previous page"):
            st.session_state.current_page -= 1
            st.rerun()
    ...
    with col4:
        if st.button("▶️", disabled=st.session_state.current_page == total_pages,
                    width="stretch", help="Next page"):
            st.session_state.current_page += 1
            st.rerun()
    
    with col5:
        if st.button("⏭️", disabled=st.session_state.current_page == total_pages,
                    width="stretch", help="Last page"):
...
def render_table_view(df: pd.DataFrame):
    ...
    if 'scId' in display_df.columns:
        display_df["_action"] = display_df["scId"].apply(
            lambda x: f"/Company_Details?sc_id={x}" if pd.notna(x) else ""
        )
...
def _render_company_card(row: pd.Series, index: int):
    ...
    with st.container(border=True):
        ...
        c1.markdown(f"<h4 style='margin:0; padding:0;'>{row.get('company_name', 'Unknown')}</h4>", unsafe_allow_html=True)
        
        status = row.get("status", "Unknown")
        status_color = "🟢" if status == "Synced" else "🟡"
        c2.markdown(f"<div style='text-align:right; padding:0; margin:0;'>{status_color}{status}</div>",
                   unsafe_allow_html=True)
        
        # Metadata
        sector = row.get('sector', 'N/A') or 'N/A'
        industry = row.get('industry', 'N/A') or 'N/A'
        st.markdown(f"<p style='margin:2px 0; font-size:0.8rem;'>📁 {sector} • {industry}</p>", unsafe_allow_html=True)
        ...
        if cost and str(cost) not in ["-", "None", "nan"]:
            ...
            st.markdown(f"<h3 style='color:{price_color}; margin:4px 0;'>₹{cost}</h3>",
                       unsafe_allow_html=True)
            if price_change_value:
                change_icon = "📉" if price_change_key == "Red" else "📈"
                st.markdown(f"<p style='margin:0; font-size:0.75rem;'>{change_icon} {price_change_value}</p>", unsafe_allow_html=True)
            ...
            if pe_line or conf_line:
                ...
                extras = ' • '.join(parts)
                st.markdown(f"<p style='margin:4px 0; font-size:0.75rem;'>{extras}</p>", unsafe_allow_html=True)
        else:
            st.markdown("<h4 style='color:#6b7280; margin:4px 0;'>--</h4>", unsafe_allow_html=True)
        ...
        with m1:
            st.markdown(f"<div style='text-align:center;'><p style='margin:0; font-size:0.75rem; color:#9ca3af;'>M-Score</p><p style='margin:2px 0; font-size:1rem; font-weight:600;'>{_format_metric(row.get('m_score'))}</p></div>", unsafe_allow_html=True)
        with m2:
            # Highlight TTM PE based on comparison with sector
            pe_val, pe_color = _format_pe_with_highlight(
                row.get('ttm_pe'),
                row.get('sector_pe'),
                row.get('pe_vs_sector_pct')
            )
            st.markdown(f"<div style='text-align:center;'><p style='margin:0; font-size:0.75rem; color:#9ca3af;'>TTM PE</p><p style='margin:2px 0; font-size:1rem; font-weight:600; color:{pe_color};'>{pe_val}</p></div>", unsafe_allow_html=True)
        with m3:
            st.markdown(f"<div style='text-align:center;'><p style='margin:0; font-size:0.75rem; color:#9ca3af;'>Analyst</p><p style='margin:2px 0; font-size:1rem; font-weight:600;'>{row.get('analyst_final_rating', '-') or '-'}</p></div>", unsafe_allow_html=True)
        ...
        with btn1:
            ...
            if sc_id:
                detail_url = f"/Company_Details?sc_id={sc_id}"
...
def main():
    ...
    if filtered_count == total_count:
        ...
        st.success(f"🎯 Showing **{filtered_count:,}** of **{total_count:,}** companies")
